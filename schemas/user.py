...
class VerifyResponse(BaseModel):
    message: str = Field(..., title="消息", description="消息", examples=["用户注册成功。"])
    user: dict = Field(
        ..., title="用户", description="用户",
        examples=[
            {
                "id": 1,
                "username": "user",
...
# 通过github第三方登录的response
class GithubLoginResponse(BaseModel):
    message: str = Field(..., title="消息", description="消息", examples=["登录成功"])
    user: dict = Field(
        ..., title="用户", description="用户",
        examples=[
            {
